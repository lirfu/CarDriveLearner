...
public class RoadGenerator extends JPanel {
	...
	public RoadGenerator(int width, int threadDelay, Dimension size) {
		this.size = size;
		this.roadWidth = width;
		this.y_step = (int) (0.9 * width);
		this.roadLineWidth = (int) (width * 0.03);
		this.threadDelay = threadDelay;
		
		roadCenterHistory = new LinkedList<>();
		
		// Starting straight.
		for (int i = 0; i < size.height / width + 2; i++)
	 ...
	 * @Override public void run() { // Starting straight. synchronized (this) { synchronized (roadCenterHistory) { while (maxPoints == 0) { repaint();
	 * calculateMaxPoints(); } for (int i = 0; i <= maxPoints + 1; i++) roadCenterHistory.add(50); repaint(); } // Initial delay. synchronized (thread) { try {
	 * thread.wait(3 * threadDelay); } catch (InterruptedException e) { e.printStackTrace(); } } } // Draw new random segments. while (!threadKill) {
	 * synchronized (this) { nextSegment(); repaint(); synchronized (thread) { try { thread.wait(threadDelay); } catch (InterruptedException e) {
	 * e.printStackTrace(); } } } } } });
	 */
	@Override
	public void paint(Graphics g) {
		...
		synchronized (roadCenterHistory) {
			...
			for (Integer percent : roadCenterHistory) {
				...
				new RoadSegment(new Point(getWidth() * last / 100, y), new Point(getWidth()
						* percent / 100, y - y_step), roadWidth, roadLineWidth).paint(g);
	...
	public void nextSegment() {
		...
		if (Math.signum(lastTurnDirectionAmount) != turnDirection
				|| Math.abs(lastTurnDirectionAmount) <= 3)
	...
	/*
	 * public void start() { if (thread.isAlive()) { System.out.println("Road generator is still alive!"); return; } threadKill = false; thread.setDaemon(true);
	...
	public boolean vehicleOffRoad(Car icon) {
		RoadSegment currentSegment =
				new RoadSegment(
						new Point(getWidth() * roadCenterHistory.get(0) / 100, getHeight()),
						new Point(getWidth() * roadCenterHistory.get(1) / 100, getHeight() - y_step),
						roadWidth, roadLineWidth);
